        ...
        {paginatedItems.map(item => {
          ...
          return (
            <div key={item.packId} className="rounded-xl overflow-hidden">
              <div className={`flex items-center gap-3 p-3 bg-blue-950/30 border border-blue-800/20 hover:border-blue-400/30 transition-colors cursor-pointer ${isExp ? 'rounded-t-xl border-b-0' : 'rounded-xl'}`} onClick={() => setExpandedPackId(isExp ? null : item.packId)}>
                <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-blue-900/40 flex items-center justify-center"><Package className="w-5 h-5 text-blue-400" /></div>
                <div className="flex-1 min-w-0">
