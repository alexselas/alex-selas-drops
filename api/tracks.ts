...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'PATCH') {
      ...
      // Validate that all items have at minimum an id and title (prevent garbage overwrite)
      for (const item of data) {
        if (!item || typeof item !== 'object' || !item.id || typeof item.id !== 'string') {
          return res.status(400).json({ error: 'Cada track debe tener un id valido' });
        }
      }
      // Safety: limit array size to prevent DoS
      if (data.length > 5000) return res.status(400).json({ error: 'Demasiados tracks' });
